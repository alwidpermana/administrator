<table class="table table-hover table-bordered table-striped nowrap align-middle text-center" id="datatable" width="100%">
	...
	<tbody>
		<?php foreach ($data as $key): ?>
			<tr>
				<td><a href="<?=base_url()?>monitoring/view_lamaran/<?=$key->lamaran_id?>" class="btn btn-icon btn-icon-only btn-outline-primary active-scale-down" target="_blank">
						<i class="bi bi-eye"></i>
					</a></td>
				<td><?=$offset++?></td>
				<td><?=$key->nip?></td>
				...
				<td><?=$key->no_rek_tes_1?></td>
				<td><?=$key->no_rek_tes_2?></td>
				<td><?=$key->jenis?></td>
				<td><?=$key->sub?></td>
				<td><?=$key->type?></td>
				<td></td>
