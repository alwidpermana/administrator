<table class="table table-bordered table-hover table-striped nowrap align-middle text-center" id="datatable" width="100%">
	<thead class="text-center">
		<tr>
			...
			<th colspan="3">Total Peserta Tes</th>
	...
	<tbody>
		<?php foreach ($data as $key): ?>
			<tr>
				...
				<td><?=$key->jenis?></td>
				<td><?=$key->tempat?></td>
				<td><?=$key->jml_laki_internal?></td>
				<td><?=$key->jml_perempuan_internal?></td>
				<td><?=$key->jml_total_internal == '0' ? '' : $key->jml_total_internal?></td>
				<td><?=$key->jml_laki_eksternal?></td>
				<td><?=$key->jml_perempuan_eksternal?></td>
				<td><?=$key->jml_total_eksternal == '0' ? '' : $key->jml_total_eksternal?></td>
				<td><?=$key->jml_laki_umum?></td>
				<td><?=$key->jml_perempuan_umum?></td>
				<td><?=$key->jml_total_umum == '0' ? '' : $key->jml_total_umum?></td>
				<td><?=$key->jml_laki_internal+$key->jml_laki_eksternal+$key->jml_laki_umum?></td>
				<td><?=$key->jml_perempuan_internal+$key->jml_perempuan_eksternal+$key->jml_perempuan_umum?></td>
				<td><?=$key->jml_perempuan_internal+$key->jml_perempuan_eksternal+$key->jml_perempuan_umum+$key->jml_laki_internal+$key->jml_laki_eksternal+$key->jml_laki_umum?></td>
