  	...
  	public function saveLowongan()
  	{
	 	...
	 	if (hash_verified($setToken,$this->session->userdata("token"))) {
	 		...
	 		if ($cekData->num_rows() == 0) {
	 			$cekLamaran = $this->db->query("SELECT id FROM lamaran WHERE pelamar_id = '$pelamarId' AND recruitment_id != '$id' AND status = 'OPEN'");
	 			if ($cekLamaran->num_rows()==0) {
	 				$lamaranId = $this->uuid->v4();
		    		$parsingData = array('id' =>$lamaranId,'pelamar_id'=>$pelamarId,'recruitment_id'=>$id,'nip'=>$nip,'tgl'=>$tanggal,'jenis'=>$jenis,'sub'=>$sub,'type'=>$type,'status_data'=>'OPEN','status'=>'Aktif','created_at'=>$tanggal,'user_id'=>$this->session->userdata("user_id"));
